...
export function createGameState(): GameState {
  return {
    bird: { x: 150, y: GAME.height / 2, vy: 0 },
    pipes: [{ x: GAME.width, gap: 180 }],
    frame: 0,
    score: 0,
    dead: false,
  };
...
export function nextPipeGap(random = Math.random) {
  return 80 + random() * (GAME.height - GAME.gap - 160);
}

export function stepGame(state: GameState, flap: boolean, random = Math.random): GameState {
  const bird = { ...state.bird, vy: flap ? GAME.lift : state.bird.vy };
  ...
  for (const pipe of pipes) {
    if (!pipe.scored && pipe.x + GAME.pipeWidth < bird.x) {
      pipe.scored = true;
      score += 1;
    }
  }
  ...
  const hitPipe = pipes.some(
    (pipe) =>
      ...
      (bird.y < pipe.gap || bird.y > pipe.gap + GAME.gap),
  );
  const dead = bird.y < 0 || bird.y > GAME.height || hitPipe;

  return { bird, pipes, frame, score, dead };
}
